...
public class MyLinkedList<AnyType> implements Iterable<AnyType> {
    ...
    private static class Node<AnyType> {
        ...
        /**
         * 构造器，根据数据、前驱节点和后继节点构造当前节点 每个节点除了存储数据外，还存储了其前驱节点和后继节点，藉此实现‘可双向遍历’链表
         * 
         * @param data 数据
         * @param pre  前驱节点
         * @param next 后继节点
     ...
     * @throw IndexOutOfBoundsException if idx is not between 0 and size() - 1
     *        ,inclusive.
     */
     ...
     * @return internal node corresponding to idx.
     * @throw IndexOutOfBoundsException if idx is not between lower and
     *        upper,inclusive.
     */
    ...
    private class LinkedListIterator implements Iterator<AnyType> {
        /**
         * 当前节点默认指向链表首部标识节点的下一节点(第一个存储数据的节点)
         */
        private Node<AnyType> current = beginMarker.next;
        /**
         * 期望的链表修改次数计数(在迭代器运行过程中，链表修改次数计数与迭代器初始化时的链表修改计数相同则表明链表在迭代器运行期间无修改，可以继续推进链表迭代器运行)
         */
        ...
        public void remove() {
            ...
            expectedModCount++;
            okToRemove = false;// 在链表数据发生变动之后，重置是否可以删除标识
        }
